...
const TABLE_COLUMNS = [
  ...
  { key: "firstRegistrationDate", label: "1st Registration" },
  { key: "hsnTsn", label: "HSN / TSN" },
  { key: "tireSize", label: "Tire Size" },
  { key: "odometer", label: "Odometer" },
  { key: "brandModel", label: "Brand / Model" },
  { key: "carPolicyAssignment", label: "Car Policies" },
  { key: "location", label: "Location" },
  { key: "costCenter", label: "Cost Center" },
  { key: "driverAssignment", label: "Driver Assignment" },
  { key: "startingTireApp", label: "Starting Tire App" },
  { key: "leasingCompany", label: "Leasing Company" },
  { key: "leasingEnd", label: "Leasing End" },
  { key: "tuvDue", label: "TUV Due" },
];
...
const normalizeFallbackVehicles = (rows) =>
  rows.map((row, index) => ({
    ...row,
    id: `fallback-vh-${index + 1}`,
    fleetId: "",
    fleetCode: "",
    fleetName: "N/A",
    fleetCity: "N/A",
...
function AdminVehicleDataPage() {
  ...
  const loadVehicles = async () => {
    try {
      setIsLoading(true);
      setPageError("");
      const vehicleRows = await listAdminVehicles();
      if (vehicleRows.length) {
        setRows(vehicleRows);
        return;
      }
      setRows(normalizeFallbackVehicles(adminVehicleDataRows));
    } catch (error) {
      setPageError(error.message || "Failed to load vehicle data from backend.");
      setRows(normalizeFallbackVehicles(adminVehicleDataRows));
    } finally {
      setIsLoading(false);
    }
  ...
  const openEditModal = (row) => {
    setFormValues({
      ...
      carPolicyAssignment: row.carPolicyAssignment || "",
      location: row.location || "",
      costCenter: row.costCenter || "",
      driverAssignment: row.driverAssignment || "",
      startingTireApp: row.startingTireApp || "",
      leasingCompany: row.leasingCompany || "",
      leasingEnd: row.leasingEnd || "",
      tuvDue: row.tuvDue || "",
    });
    setTargetRow(row);
  ...
  const deleteSelected = async () => {
    ...
    try {
      setIsSubmitting(true);
      await Promise.all(selectedIds.map((id) => deleteAdminVehicle(id)));
      setRows((current) => current.filter((row) => !selectedIds.includes(row.id)));
      setSelectedIds([]);
    } catch (error) {
      setPageError(error.message || "Failed to delete selected vehicles.");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const saveNewEntry = async () => {
    if (!formValues.vin.trim() || !formValues.licensePlate.trim()) return;

    try {
      setIsSubmitting(true);
      const created = await createAdminVehicle(formValues);
      setRows((current) => [created, ...current]);
      closeEntryModal();
      setPage(1);
    } catch (error) {
      setPageError(error.message || "Failed to create vehicle.");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const saveEditEntry = async () => {
    if (!targetRow?.id || !formValues.vin.trim() || !formValues.licensePlate.trim()) return;

    try {
      setIsSubmitting(true);
      const updated = await updateAdminVehicle(targetRow.id, formValues);
      setRows((current) => current.map((row) => (row.id === targetRow.id ? updated : row)));
      closeEditModal();
    } catch (error) {
      setPageError(error.message || "Failed to update vehicle.");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const importBulkData = async () => {
    ...
    try {
      setIsSubmitting(true);
      const result = await bulkUploadAdminVehicles(bulkFile);
  ...
  return (
    <section className="relative rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <ApiCallLoaderOverlay
        show={isSubmitting}
        title="Updating vehicle data"
        subtitle="Please wait while we process your request."
      />

      <h2 className="text-lg font-semibold text-slate-900 sm:text-xl">Vehicles</h2>
      <p className="mt-2 text-sm text-slate-500">Manage vehicle records.</p>
      ...
      <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="relative min-w-[220px] flex-1">
            ...
            <input
              ...
              placeholder="Search by VIN, plate, model, fleet..."
              type="text"
              value={search}
            />
          </div>
          ...
          >
            ...
            {fleetFilterOptions.map((fleet) => (
              <option key={fleet.id} value={fleet.id}>
                {fleet.name}{fleet.code ? ` (${fleet.code})` : ""}
              </option>
            ))}
          </select>
        ...
        <div className="mt-3 overflow-x-auto rounded-xl border border-slate-200">
          <table className="min-w-[1960px] divide-y divide-slate-200 text-sm">
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {isLoading ? (
                <TableLoadingRow
                  ...
                  title="Loading vehicle records..."
                  subtitle="Please wait while we sync latest vehicle data."
                />
              ) : null}
                    ...
                    <tr className="hover:bg-slate-50/70" key={row.id}>
                      ...
                      <td className="px-3 py-2">
                        <div className="relative">
                          <button
                            aria-label={`Open actions for ${row.licensePlate}`}
              ...
              {!isLoading && !pagedRows.length ? (
                <tr>
                  <td className="px-3 py-8 text-center text-sm text-slate-500" colSpan={TABLE_COLUMNS.length + 3}>
                    No vehicle records found for current filters.
                  </td>
                </tr>
              ) : null}
            </tbody>
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
          <p className="text-xs text-slate-500">
            Showing {pagedRows.length} of {filteredRows.length} vehicle records
          </p>
          <div className="flex items-center gap-2">
            <label className="text-xs text-slate-500" htmlFor="vehicle-page-size">
              Rows
            </label>
            <select
      ...
      {isDeleteModalOpen ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/50 p-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-slate-900">Delete vehicle record?</h3>
            <p className="mt-2 text-sm text-slate-500">
              Are you sure you want to delete{" "}
              <span className="font-semibold text-slate-700">
                {targetRow?.licensePlate || "this vehicle"}
              </span>
          ...
          <div className="w-full max-w-6xl rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-lg font-semibold text-slate-900">
                {isEditModalOpen ? "Edit vehicle data" : "Add vehicle data"}
              </h3>
              <button
            ...
            <p className="mt-2 text-sm text-slate-500">
              Fill vehicle fields and save to update the master dataset.
            </p>
      ...
      {isUploadModalOpen ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/50 p-4">
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-slate-900">Bulk upload vehicle data</h3>
            <p className="mt-2 text-sm text-slate-500">
              Upload CSV, XLS, or XLSX file to add multiple vehicle records.
            </p>
