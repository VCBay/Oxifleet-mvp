...
import { adminCarPolicyRows } from "../data/adminStore";
import ApiCallLoaderOverlay from "../components/common/ApiCallLoaderOverlay";
import TableLoadingRow from "../components/common/TableLoadingRow";
import {
  bulkUploadAdminCarPolicies,
  createAdminCarPolicy,
  deleteAdminCarPolicy,
  listAdminCarPolicies,
  updateAdminCarPolicy,
} from "../services/adminCarPolicyApi";

const PAGE_SIZE_OPTIONS = [5, 8, 10, 20];

const TABLE_COLUMNS = [
  { key: "policyCode", label: "Policy Code" },
  { key: "policyVersion", label: "Version" },
  { key: "policyScope", label: "Policy Scope" },
  { key: "approvedTiresPerOem", label: "Approved Tires per OEM" },
  { key: "preferredTires", label: "Preferred Tires" },
  { key: "approvedServiceNetwork", label: "Approved Service Network" },
  { key: "priorityServiceNetwork", label: "Priority Service Network" },
  { key: "discountsPerOem", label: "Discounts per OEM" },
  { key: "replacementIntervals", label: "Replacement Intervals" },
  { key: "replacementReminderRules", label: "Reminder Rules" },
  { key: "approvalRules", label: "Approval Rules" },
  { key: "orderInvoiceRules", label: "Order/Invoice Rules" },
  { key: "cashbackPerOem", label: "Cashback per OEM" },
];
...
const INITIAL_FORM_STATE = {
  ...
  approvedTiresPerOem: "",
  preferredTires: "",
  approvedServiceNetwork: "",
  priorityServiceNetwork: "",
  discountsPerOem: "",
  replacementIntervals: "",
  replacementReminderRules: "",
  approvalRules: "",
  orderInvoiceRules: "",
  cashbackPerOem: "",
};

const normalizeFallbackPolicies = (rows) =>
  rows.map((row, index) => ({
    ...row,
    id: row.id || `fallback-pol-${index + 1}`,
  }));

function AdminCarPolicyPage() {
  ...
  const loadCarPolicies = async () => {
    try {
      setIsLoading(true);
      setPageError("");
      const policyRows = await listAdminCarPolicies();
      if (policyRows.length) {
        setRows(policyRows);
        return;
      }
      setRows(normalizeFallbackPolicies(adminCarPolicyRows));
    } catch (error) {
      setPageError(error.message || "Failed to load car policy data from backend.");
      setRows(normalizeFallbackPolicies(adminCarPolicyRows));
    } finally {
      setIsLoading(false);
    }
  ...
  const openEditModal = (row) => {
    setFormValues({
      ...
      approvedTiresPerOem: row.approvedTiresPerOem || "",
      preferredTires: row.preferredTires || "",
      approvedServiceNetwork: row.approvedServiceNetwork || "",
      priorityServiceNetwork: row.priorityServiceNetwork || "",
      discountsPerOem: row.discountsPerOem || "",
      replacementIntervals: row.replacementIntervals || "",
      replacementReminderRules: row.replacementReminderRules || "",
      approvalRules: row.approvalRules || "",
      orderInvoiceRules: row.orderInvoiceRules || "",
      cashbackPerOem: row.cashbackPerOem || "",
    });
    setTargetRow(row);
  ...
  const deleteSelected = async () => {
    ...
    try {
      setIsSubmitting(true);
      await Promise.all(selectedIds.map((id) => deleteAdminCarPolicy(id)));
      setRows((current) => current.filter((row) => !selectedIds.includes(row.id)));
      setSelectedIds([]);
    } catch (error) {
      setPageError(error.message || "Failed to delete selected car policies.");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const saveNewEntry = async () => {
    if (!formValues.policyCode.trim() || !formValues.policyScope.trim()) return;

    try {
      setIsSubmitting(true);
      const created = await createAdminCarPolicy(formValues);
      setRows((current) => [created, ...current]);
      closeEntryModal();
      setPage(1);
    } catch (error) {
      setPageError(error.message || "Failed to create car policy.");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const saveEditEntry = async () => {
    if (!targetRow?.id || !formValues.policyCode.trim() || !formValues.policyScope.trim()) return;

    try {
      setIsSubmitting(true);
      const updated = await updateAdminCarPolicy(targetRow.id, formValues);
      setRows((current) => current.map((row) => (row.id === targetRow.id ? updated : row)));
      closeEditModal();
    } catch (error) {
      setPageError(error.message || "Failed to update car policy.");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const importBulkData = async () => {
    ...
    try {
      setIsSubmitting(true);
      const result = await bulkUploadAdminCarPolicies(bulkFile);
  ...
  return (
    <section className="relative rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <ApiCallLoaderOverlay
        show={isSubmitting}
        title="Updating car policy data"
        subtitle="Please wait while we process your request."
      />

      <h2 className="text-lg font-semibold text-slate-900 sm:text-xl">Car Policies</h2>
      <p className="mt-2 text-sm text-slate-500">Configure and manage fleet car policies.</p>
      ...
      <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="relative min-w-[220px] flex-1">
            ...
            <input
              ...
              placeholder="Search by policy code, scope, OEM, approval rules..."
              type="text"
              value={search}
            />
          </div>
        ...
        <div className="mt-3 overflow-x-auto rounded-xl border border-slate-200">
          <table className="min-w-[1920px] divide-y divide-slate-200 text-sm">
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {isLoading ? (
                <TableLoadingRow
                  ...
                  title="Loading car policy records..."
                  subtitle="Please wait while we sync latest policy data."
                />
              ) : null}
                    ...
                    <tr className="hover:bg-slate-50/70" key={row.id}>
                      ...
                      <td className="px-3 py-2">
                        <div className="relative">
                          <button
                            aria-label={`Open actions for ${row.policyCode}`}
              ...
              {!isLoading && !pagedRows.length ? (
                <tr>
                  <td className="px-3 py-8 text-center text-sm text-slate-500" colSpan={TABLE_COLUMNS.length + 2}>
                    No policy records found for current search.
                  </td>
                </tr>
              ) : null}
            </tbody>
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
          <p className="text-xs text-slate-500">
            Showing {pagedRows.length} of {filteredRows.length} policy records
          </p>
          <div className="flex items-center gap-2">
            <label className="text-xs text-slate-500" htmlFor="policy-page-size">
              Rows
            </label>
            <select
      ...
      {isDeleteModalOpen ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/50 p-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-slate-900">Delete policy record?</h3>
            <p className="mt-2 text-sm text-slate-500">
              Are you sure you want to delete <span className="font-semibold text-slate-700">{targetRow?.policyCode || "this policy"}</span>?
          ...
          <div className="w-full max-w-6xl rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-lg font-semibold text-slate-900">
                {isEditModalOpen ? "Edit car policy" : "Add car policy"}
              </h3>
              <button
            ...
            <p className="mt-2 text-sm text-slate-500">Fill policy fields and save to update the policy dataset.</p>
      ...
      {isUploadModalOpen ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/50 p-4">
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
            <h3 className="text-lg font-semibold text-slate-900">Bulk upload car policy data</h3>
            <p className="mt-2 text-sm text-slate-500">Upload CSV, XLS, or XLSX file to add multiple policy records.</p>
...
export default AdminCarPolicyPage;
