...
import { completeTwoFactorSetup, initiateTwoFactorSetup } from "../services/authApi";
...
function TwoFactorAuthPage() {
  ...
  const contactHint = useMemo(
    () => session?.phone || "No mobile number in profile.",
    [session?.phone],
  );
  ...
  if (!requiresTwoFactorSetup(session)) {
    return <Navigate replace to={getPostTwoFactorRoute(session)} />;
  }
  ...
  const handleSendOtp = async () => {
    ...
    try {
      const result = await initiateTwoFactorSetup("sms");
      setChallengeId(result?.challengeId || "");
      setResendCooldown(Number(result?.resendCooldownSeconds || 30));
      setSuccessMessage(
        `OTP sent via SMS to ${result?.recipientMasked || contactHint}.`,
      );
    } catch (sendError) {
  ...
  const handleVerify = async (event) => {
    ...
    try {
      ...
      if (session?.token) {
        ...
        if (result?.user) {
          updatedSession = {
            ...
            token: session.token,
            role: session.role,
            type: result.user.type || session.type,
            name: session.name,
            twoFactorVerified: true,
          };
        }
      ...
      navigate(getPostTwoFactorRoute(updatedSession), { replace: true });
    } catch (verifyError) {
      setError(verifyError?.message || "Unable to complete 2FA setup.");
  ...
  const resendButtonLabel =
    resendCooldown > 0 ? `Resend OTP in ${formatSeconds(resendCooldown)}` : "Send OTP";
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[linear-gradient(180deg,#0f0a1e_0%,#1f1240_50%,#2a1656_100%)] px-4 py-8 sm:px-6">
          ...
          <div className="mb-6 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-600">
                Mandatory setup. Verify OTP on your mobile number to continue.
              </p>
            </div>
          </div>
      ...
      {showAgreementModal ? (
        <div className="fixed inset-0 z-20 grid place-items-center bg-[#0b0520]/65 p-4">
          <div className="w-full max-w-md rounded-3xl border border-white/20 bg-white p-6 shadow-2xl">
            <h2 className="text-xl font-semibold text-[#1a1038]">2FA setup is mandatory</h2>
            <p className="mt-2 text-sm text-slate-600">
              For account security, complete two-factor authentication now. You cannot continue
              without it.
            </p>
            <button
              className="mt-5 w-full rounded-xl bg-[linear-gradient(180deg,#4a2ba1_0%,#2a1656_100%)] px-4 py-2.5 text-sm font-semibold text-white transition hover:opacity-95"
